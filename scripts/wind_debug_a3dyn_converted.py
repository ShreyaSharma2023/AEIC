...
def _open_converted(path, *args, **kwargs):
    ds = _original_open_dataset(path, *args, **kwargs)
    # Already has correct coord/var names. A3dyn's 8 timesteps are 3-hour
    # AVERAGES centered on the half-hour (01:30, 04:30, ...), not exact-hour
    # instantaneous values -- so 'nearest' (whichever bin's window actually
    # covers a given hour) is the physically correct reindex method here,
    # not 'ffill' (which also leaves hour 0 unfilled, since it can't
    # backfill before the first bin-center at 01:30).
...
_weather_module.xr.open_dataset = _open_converted
